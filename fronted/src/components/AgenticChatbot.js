...
const AgenticChatbot = ({ userId }) => {
  ...
  const sendMessage = async () => {
    ...
    try {
      let apiUrl = `${process.env.REACT_APP_API_BASE_URL}/api/agent-chat`;
      let res = await fetch(apiUrl, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ message: input, userId }),
      });
      // If Railway returns 404, fallback to local
      if (res.status === 404 && window.location.hostname === 'localhost') {
        apiUrl = `${process.env.REACT_APP_API_BASE_URL}/api/agent-chat`;
